...
<script>
  ...
  import InciInThePress from "../components/InciInThePress.vue";
  ...
  export default {
    name: "press",
    layout: "default",
    components: { LittleHero, InciInThePress, MediaSidebar },
    async asyncData({ i18n, route }) {
      ...
      var pressResult = await mediaService.getPress(pageFilter);
      var pressData = pressResult.result;

      var pageCount = parseInt(Math.ceil(pressResult.totalCount / pageFilter.limit));


      return { pageSettings, sideMenu, pressData, pageCount, currentPage };
    },
    methods: {
      getFullPage(i) {
